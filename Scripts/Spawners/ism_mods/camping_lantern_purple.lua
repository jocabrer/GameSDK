...
local newItem = {
  class = "camping_lantern_purple", -- the item class name needs to be the same as the one in the xml file
  percent = 5, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomCampingBPart")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "class", "camping_pop_up_canopy") -- could also search for fitting "class" instead "category"
